...
// Only public, indexable destinations here: the footer is on every page, so links to signed-in
// routes would hand crawlers a trail of noindex URLs.
const COLUMNS = [
  {
    heading: "Product",
    links: [
  ...
  },
  {
    ...
    links: [
      { href: "/#features", label: "QuickBooks Online sync" },
      { href: "/#features", label: "Tax CSV export" },
      { href: "/#features", label: "Mobile capture" },
      { href: "/#features", label: "Multi-currency" },
    ],
  },
];
...
export default function Footer() {
  ...
  const headingClass = `text-xs font-semibold uppercase tracking-wider mb-4 ${isDark ? "text-gray-300" : "text-gray-700"}`;
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
          ...
          <div className="col-span-2 md:col-span-2">
            <Link href="/" aria-label="Tallyhawk home" className={`inline-flex mb-4 ${isDark ? "text-white" : "text-gray-900"}`}>
              <Logo />
            </Link>
            <p className={`text-sm leading-relaxed max-w-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>
              AI-powered receipt and invoice processing. Stop doing manual data entry and get back to building your business.
            </p>
            <div className="flex items-center gap-4 mt-6">
              <a href={SOCIAL_LINKS.github} target="_blank" rel="noopener noreferrer" aria-label="Tallyhawk on GitHub" className={linkClass}>
            ...
            <nav key={column.heading} aria-label={column.heading}>
              ...
              <ul className="space-y-3">
                {column.links.map((link) => (
                  <li key={link.label}><Link href={link.href} className={linkClass}>{link.label}</Link></li>
                ))}
              </ul>
            </nav>
          ))}
